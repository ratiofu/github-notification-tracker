...
export function parseOptionalJsonRow<ParsedValue>(
  row: unknown,
  schema: { parse(value: unknown): ParsedValue },
...
export function parseJsonRow<ParsedValue>(
  row: unknown,
  schema: { parse(value: unknown): ParsedValue },
): ParsedValue {
...
export function readString(row: unknown, key: string): string {
  ...
  if (typeof value !== "string") {
    throw new TypeError(`SQLite row ${key} is not a string`)
  }
